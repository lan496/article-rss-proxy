...
1. **Fetch** (`fetcher/chemrxiv_fetcher.py`): Fetches preprint metadata from the Crossref REST API (DOI prefix `10.26434`) because chemrxiv.org itself is behind a Cloudflare JS challenge that blocks all plain HTTP clients. Filters by posted date, dedupes revision DOIs (`.../vN`), strips JATS markup from abstracts.
2. **Filter** (`llm_utils.py` → `recommend_papers`): No subject-area filter is available via Crossref, so all daily preprints (~20-40) go through the LLM recommendation pass.
3. **Generate RSS** (`rss_generator.py`): Produces `docs/chemrxiv.xml`.
